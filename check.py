#!/usr/bin/env python3
# ===================================================================
# PREMIUM DEVID SEKER - ULTRA MLBB TOOLS v7.0
# Created by: @PRIME_MICK
# Features: OEM A/B/C/D + 5 Real Formats + Live Check + TG Report
# ===================================================================
...
{Fore.YELLOW}═══════════════════════════════════════════════════════════════
{Fore.LIGHTBLACK_EX}              Created by: {Fore.CYAN}{ADMIN_USERNAME_DISPLAY}{Fore.LIGHTBLACK_EX} | v7.0
...
# ────────────────────────────────────────────────────────────────
# 8. DEVICE GENERATOR — 5 Real Formats
# ────────────────────────────────────────────────────────────────
...
# ── Format 1: Main (Highest hit) ──
def oem_d_format_1():
    """and_{32hex}{20b36}-{UUID_4part}"""
    return f"and_{random_hex(32)}{random_base36(20)}-{str(uuid.uuid4())[9:]}"


# ── Format 2 ──
def oem_d_format_2():
    """and_{32hex}{24b36}-{UUID_4part}"""
    return f"and_{random_hex(32)}{random_base36(24)}-{str(uuid.uuid4())[9:]}"


# ── Format 3 ──
def oem_d_format_3():
    """and_{32hex}{25b36}-{UUID_4part}"""
    return f"and_{random_hex(32)}{random_base36(25)}-{str(uuid.uuid4())[9:]}"


# ── Format 4: iOS Simple ──
def oem_d_format_4():
    """ios_{UUID_UPPER}"""
    return f"ios_{str(uuid.uuid4()).upper()}"


# ── Format 5: iOS + Timestamp ──
def oem_d_format_5():
    """ios_{UUID_UPPER}_{timestamp}"""
    ts = int(time.time() * 1000000)
    return f"ios_{str(uuid.uuid4()).upper()}_{ts}"


# ── Format pool (weighted) ──
OEM_D_FORMATS = (
    [oem_d_format_1] * 6 +   # 60% - Highest hit
    [oem_d_format_2] * 2 +   # 20%
    [oem_d_format_3] * 1 +   # 10%
    [oem_d_format_4] * 1 +   # 5%
    [oem_d_format_5] * 1     # 5%
...
def generate_oem_d() -> str:
    """OEM D — Random from 5 real working formats"""
    return random.choice(OEM_D_FORMATS)()


def generate_verified_device() -> str:
    """Generate based on current OEM mode"""
    mode = OEM_MODE
    ...
    if mode == "D":
        ...
        if r < 0.30:
            ...
            oem = random.choice(OEM_B_POOL)
            return f"and_{oem}{random_hex(16)}-{uuid.uuid4()}"
        else:
            oem = random.choice(OEM_C_POOL)
            return f"and_{oem}{random_hex(16)}-{uuid.uuid4()}"
...
def run_generator_menu():
    ...
    print(f"{Fore.YELLOW}[4]{Style.RESET_ALL} {Fore.YELLOW}⭐ OEM D{Style.RESET_ALL} — REAL WORKING (5 Formats)")
    print(f"{Fore.LIGHTBLACK_EX}     Format 1: and_{{32hex}}{{20b36}}-{{UUID4}}  (60%){Style.RESET_ALL}")
    print(f"{Fore.LIGHTBLACK_EX}     Format 2: and_{{32hex}}{{24b36}}-{{UUID4}}  (20%){Style.RESET_ALL}")
    print(f"{Fore.LIGHTBLACK_EX}     Format 3: and_{{32hex}}{{25b36}}-{{UUID4}}  (10%){Style.RESET_ALL}")
    print(f"{Fore.LIGHTBLACK_EX}     Format 4: ios_{{UUID_UPPER}}             (5%){Style.RESET_ALL}")
    print(f"{Fore.LIGHTBLACK_EX}     Format 5: ios_{{UUID_UPPER}}_{{ts}}        (5%){Style.RESET_ALL}\n")
    ...
    if OEM_MODE == "D":
        print(f"{Fore.YELLOW}Sample OEM D devices:{Style.RESET_ALL}")
        for i in range(3):
            print(f"  {Fore.LIGHTBLACK_EX}{generate_oem_d()}{Style.RESET_ALL}")
        print()
